...
(define (deNero x)
  (define x 1)
  (if (or 0 (/ 1 'announcements!))
    (begin (define x 'cs61a) (print x))
))

(define john deNero)
(john (quote '(buffalo buffalo buffalo buffalo)))
; expect cs61a


(define hilfinger (mu (hilfinger n) n))
(let ( (paul hilfinger) (n 'RTFM) (self-proclaimed 'terror_of_CS) )
     (paul 'cs61bFa2018 'mercy_please)
)
; expect mercy_please
...
(let ((zzz (hilfinger 20 18)))
  (omae zzz))
; expect shindeiru

(define (internal universe)
  (define (darkness within)
    (define overeagerness (mu (moo) vyzin))
    (overeagerness (and 24 70))
  )
  (define vyzin 3)
  darkness
)

(cons (* ( (internal 98) 3) 1.5) (cons 'uno-mas 'nil))
; expect (4.5 uno-mas)

(define no 'eyes)
(define enlightened #t)
(begin (begin 'hello-reader) (cond ((eq? 'eyes no) (define primitive +) (define (adder a b) (/ (primitive a b) 5)) (let ((a 3) (b 7)) (adder a b)))
(else (if (not enlightened)'I see now that the circumstances of one's birth are irrelevant. It is what you do with the gift of life that determines who you are.'
'i'm running out of things to put))))
; expect 2

(define (make-sadder person mood)
  (define (helper)
    (weak-move)
    (begin (define strong-move 'sad) (define mood strong-move))
    mood)
  (define (weak-move)
    (define mood 'happy))
  helper
...
((make-sadder 'bob 'ecstatic))
; expect sad

(define A-plus 293)
(define (61a-grade grade)
  (define (hack_grades grade) (primitive 50 grade))
  (if (>= grade A-plus)
    ( quote thanks_obama )
    (61a-grade (hack_grades grade))
  )
)
(61a-grade 12)
;expect thanks_obama
...
(define (def x) x)
(define thefine 'define)
(def (define define thefine) )
;expect define
