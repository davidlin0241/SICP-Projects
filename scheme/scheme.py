...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        #NOT ^ because if expr had nested expressions like
        #Pair('+', Pair( (- 5 3), Pair(2, nil)))
        #the code behind scheme_apply would not work properly because the assunption is made that
        #operands should be an already evaulated pair like Pair('+', Pair(2, Pair(2, nil)))
        #so we do the folowing to evaluate/make sure all items in the pair are evaluated (should all be self_evaluating now)
...
def eval_all(expressions, env):
    ...
    #tail context
    #written this way for problem 20 "less clean" than original
    # END PROBLEM 8
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('The number of values does not match the number of parameters')
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try: #"Call self.fn on all of those arguments using *args notation." ???
            return self.fn(*python_args) #* unpacks list?
        except TypeError:
            raise SchemeError
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        #(define size 2) expressions.second is (2), so need to go deeper (remove the outer pair)
        #(define x (+ 2 3)) expressions.second is ( (+ 2 3) )
        unnested_second = expressions.second.first
        env.define( target, scheme_eval(unnested_second, env) )
        return target
        # END PROBLEM 6
        ...
        env.define(target.first, new_lambda)
        return target.first
        # END PROBLEM 10
    else:
...
def do_or_form(expressions, env):
    ...
    while expressions.second is not nil:
        ...
        if scheme_truep( evaluated_expr ):
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            # BEGIN PROBLEM 14
            if clause.second is not nil:            #clause.second
                return eval_all(clause.second, env) #should be nested by eval_all implementation
            else:
                return test
            # END PROBLEM 14
        expressions = expressions.second
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        all_formals = Pair( binding.first, all_formals )
        all_values = Pair( scheme_eval(binding.second.first, env), all_values )

        bindings = bindings.second

    check_formals(all_formals)
    child = env.make_child_frame(all_formals, all_values)
    return child
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        ...
        new_macro = MacroProcedure(formals, body, env)
        env.define(name, new_macro)
