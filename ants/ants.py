    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if isinstance(insect, QueenAnt):
                if insect == insect.queen_ant_instances[0]:
                    return
            # END Problem 13
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem 7
        ant_or_none = self.place.ant
        if ant_or_none is not None:
            return ant_or_none.blocks_path
        else:
            return False
        # END Problem 7
    ...
    def action(self, colony):
        ...
        # BEGIN EC
        if self.move_forward is False and not self.scared:
            destination = self.place.entrance #move backwards
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        #avoid iterating while mutating list
        #by copying the list
        #however, the each bee in the copied list is the SAME bee as the original list (identity)
        # END Problem 5
...
class WallAnt(Ant):
    ...
    """
    #damage 0 by defaut (Insect superclass attribute)
    name = 'Wall'
    implemented = True
    food_cost = 4
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 7
        bees_list = [] #warned against iterating over a list and changing the contents at the same time (may not visit all elements)
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower is a more costly but watersafe ThrowerAnt."""
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.digesting != 0:
            ...
            place = self.place
            bee_or_none = random_or_none(place.bees)
            if bee_or_none != None:
                place.bees.remove(bee_or_none)
                self.eat_bee(bee_or_none)
        # END Problem 6
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        # BEGIN Problem 10
        NinjaAnt.action(self,colony) #does the same thing as NinjaAnt with the additional action below as necessary
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        """
        # BEGIN Problem 13
        if self == QueenAnt.queen_ant_instances[0]: #same as self.queen.. bbt clearer
            ThrowerAnt.action(self, colony)

            place = self.place.exit #behind QueenAnt
            while place is not None:
                ant = place.ant
                if ant is not None:
                    if ant not in self.empowered_ants:
                        ant.damage *= 2
                        self.empowered_ants += [ant]
                    if ant.is_container:
                        contained_ant = ant.contained_ant
                        if contained_ant is not None and contained_ant not in self.empowered_ants:
                            contained_ant.damage *= 2
                            self.empowered_ants += [contained_ant]
                place = place.exit
        else:
    ...
    def reduce_armor(self, amount):
        ...
        if self == QueenAnt.queen_ant_instances[0] and self.armor <= 0:
            bees_win()
        # END Problem 13
...
def apply_effect(effect, bee, duration):
    ...
    original_action, affected_action = bee.action, effect(bee.action, bee)

    def decide_action(colony):
        nonlocal duration
        ...
        if duration > 0:
            ...
            if bee.move_forward is False:
                bee.move_forward = True #reset
                bee.scared = True #never affected by scare ever again

            original_action(colony) #call the original action for the turn
            bee.action = original_action #reset

    bee.action = decide_action
    # END Problem EC
